package com.example.oBootMybatis01.handler;
...
public class SocketHandler extends TextWebSocketHandler {
	...
	// handle TextMessage 메소드는 메세지를 수신하면 실행
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		// 전체 Message 
			case "message":
				...
				if (yourName.equals("ALL")) {
				...
				} else { // 개인전송 대상자(yourName은 개인 sessionID)
					//상대방
					System.out.println("개인전송대상자 상대방 sessionID : " + yourName);
					...
					System.out.println("개인전송대상자 나  : " + meName);
					try {
		...
		// sessionUserMap에 User등록
			case "userSave":
				...
				System.out.println("==2. sessionId : sessionUserMap.sessionID         =");
				System.out.println("==3. sessionName : sessionUserMap.userName        =");
